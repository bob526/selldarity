$("#registerSubmit").click(() => {
  ...
  if (checkInputData(email, reEmail, password, userName)) {
    $.post(baseUrl+"authority/ajaxRegister", {email: email, password: password, userName: userName}, function(rtn) {
      if(rtn === 1) {
...
$("#loginSubmit").click(() => {
  var email = $("#loginEmail").val(), password = $("#loginPassword").val();
  $.post(baseUrl+"Authority/ajaxLogin", {email: email, password: password}, function(rtn) {
    if (rtn == 2) {
      $("#loginNotification").html("無此帳號，請註冊後再登入");
    } else if (rtn == 3) {
      $("#loginNotification").html("密碼錯誤");
    } else if(rtn == 4) {
      $("#loginNotification").html("請先驗證在登入");
    } else {
